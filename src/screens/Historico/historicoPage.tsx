...
const mapStateAulasHoje = (navigation): any => ({
  ...
  aguarde: { text: "Aguarde o horário da aula", color: "#96948E", functionCallback: () => {} },
});
...
const HistoricoPage = () => {
  ...
  const handleClassesToday = (today: any, setAulasHoje: any, userInfo: any) => {
    if (!!today) {
      if (
        ...
        if (jaPassou) {
          jaPassou.presente ? setAulasHoje("presente") : setAulasHoje("ausente");
        } else {
  ...
  return (
    <Container>
      {aulasHoje != "" && <DinamicHeader data={{ ...mapStateAulasHoje(navigation)[aulasHoje] }} />}
